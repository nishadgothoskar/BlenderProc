...
occupancy_grid_material = d["occupancy_grid_material"]
occupancy_grid_pose = d["occupancy_grid_pose"]
occupancy_grid_mesh_file_path = d["occupancy_grid_mesh_file_path"]
agent_poses = d["agent_poses"]
object_materials = d["object_materials"]
alternate_pose = d["alternate_pose"]
...
for i in range(num_objects):
    print(object_model_paths[object_types[i]])
...
path = occupancy_grid_mesh_file_path[0]
print(path)
occ_grid = bproc.loader.load_obj(path)[0]
occ_grid.enable_rigidbody(True, friction = 100.0, linear_damping = 0.99, angular_damping = 0.99)
occ_grid.set_shading_mode('auto')
occ_grid.set_local2world_mat(
    occupancy_grid_pose
)
...
texture_name = occupancy_grid_material
tex = bproc.filter.one_by_attr(cc_textures, "name", texture_name)
occ_grid.replace_materials(tex)
...
if alternate_pose is not None:
    cam2world = bproc.math.change_source_coordinate_frame_of_transformation_matrix(alternate_pose, ["X", "-Y", "-Z"])
    bproc.camera.add_camera_pose(cam2world)
